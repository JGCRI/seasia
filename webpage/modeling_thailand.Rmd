...
thailand <- read.csv("../data/Thailand/gcamextractor/All/gcamDataTable_aggClass1.csv") %>%
  filter(x >= 2020, x <= 2065,
         !grepl("International", class))

thailand_with_hist <- read.csv("../data/Thailand/gcamextractor/All/gcamDataTable_aggClass1.csv") %>%
  filter(x <= 2065)

scenRef_name <- "Ref_cities"
```
...
building_sector_colors <- c(
  "comm cooking" = "#b2e2e2",
  "comm hvac" = "#66c2a4",
  "comm lighting" = "#2ca25f",
  "Commercial Others" = "#006d2c",
  "resid rural air conditioners" = "#fee8c8",
  "resid rural cooking" = "#fdd49e",
  "resid rural lighting" = "#fdbb84",
  "resid rural other appliances" = "#fc8d59",
  "resid rural refrigerators" = "#ef6548",
  "resid rural televisions" = "#d7301f",
  "resid rural ventilation" = "#b30000",
  "resid rural water heaters" = "#7f0000",
  "resid urban air conditioners" = "#ece7f2",
  "resid urban cooking" = "#d0d1e6",
  "resid urban lighting" = "#a6bddb",
  "resid urban other appliances" = "#74a9cf",
  "resid urban refrigerators" = "#3690c0",
  "resid urban televisions" = "#0570b0",
  "resid urban ventilation" = "#045a8d",
  "resid urban water heaters" = "#023858" )
...
```{r building-details, results = 'hide', eval = TRUE, echo = TRUE, warning = FALSE, error = FALSE, message = FALSE, fig.width = 21, fig.height = 12, out.width = "100%"}
building_cook_params <- c("serviceOutputByTechBuildings")

building_tech_colors <- c(
  "comm cooking (gas)" = "#2ca25f",
  "comm cooking (refined liquids)" = "#006d2c",
  "resid rural cooking (biomass)" = "#fdcc8a",
  "resid rural cooking (traditional biomass)" = "#fc8d59",
  "resid rural cooking (refined liquids)" = "#e34a33",
  "resid rural cooking (electricity)" = "#b30000",
  "resid urban cooking (biomass)" = "#bdc9e1",
  "resid urban cooking (traditional biomass)" = "#74a9cf",
  "resid urban cooking (refined liquids)" = "#2b8cbe",
  "resid urban cooking (electricity)" = "#045a8d" )

pop <- thailand %>% 
  filter(param == "pop", region %in% c("Thailand", "MEA area"), scenario == scenRef_name) %>% 
  mutate(region = case_when(region == "Thailand" ~ "All of Thailand",
                            T ~ region)) %>%
  rename(pop = value) %>% 
  select(region, x, pop)

thailand_cook <- thailand %>% 
  filter(param %in% building_cook_params,
         grepl("cook", class),
         region %in% c("All of Thailand", "MEA area"))

thailand_cook2 <- thailand_cook %>% 
  full_join(pop) %>% 
  mutate(serviceOutputByTechBuildingsPerCapita = value/pop) %>% 
  select(-c(param, value, pop)) %>% 
  tidyr::pivot_longer(c(serviceOutputByTechBuildingsPerCapita),
               names_to = "param", values_to = "value") %>% 
  full_join(thailand_cook)

building_cook <- thailand_cook2  %>% 
  mutate(param = case_when(param == "serviceOutputByTechBuildings" ~ "Cooking output (EJ)",
                           T ~ "Per capita cooking output (EJ)")) %>% 
  rchart::chart(save = F, show = F, size_text = 19, scenRef = scenRef_name,
                palette = building_tech_colors)

building_cook$`chart_class_All of Thailand`
