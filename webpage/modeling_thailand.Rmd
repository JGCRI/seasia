...
dt_url <- c("https://github.com/JGCRI/seasia/blob/main/modeling/thailand/configuration_files/configuration_seasia_cities_bau.xml",
            ...
            "https://github.com/JGCRI/seasia/blob/main/modeling/thailand/configuration_files/configuration_seasia_cities_high_mea_high_rot.xml")
...
Please use the link below to download the output database from GCAM. This database contains model results for the scenarios described above.

```{r db download table, results = 'show', eval=TRUE, echo=FALSE, warning=FALSE, error = FALSE, message = FALSE}
library(kableExtra); library(dplyr)

dt_url <- c("https://zenodo.org/record/7199955/files/database_seasia_cities.zip?download=1")

data.frame(
  File = c("database_seasia_cities")) %>% 
  dplyr::mutate(Location = cell_spec(rep("Link",length(dt_url)),"html",link=dt_url)) %>%
  kable("html", escape = FALSE) %>%
  kable_styling(bootstrap_options = c("bordered","striped", "hover", "condensed", "responsive")) %>%
  row_spec(0, background = "#2A2A2A", color = "white")
...
```{r gcamextractor, echo = FALSE, eval = FALSE, class.source = 'fold-show'}
...
```{r gcamextractor single db, echo = TRUE}
thailand <- read.csv("../data/Thailand/all_data.csv")

thailand_with_hist <- read.csv("../data/Thailand/all_data_with_hist.csv")
...
thailand <- thailand %>% 
  mutate(units = case_when(param == "pop" ~ "Population (millions)",
                           param == "gdp" ~ "GDP (Billion 1990 USD)",
                           param == "gdpPerCapita" ~ "GDP per Capita (Billion 1990 USD per person)",
                           ...
                           param == "emissCO2BySectorNoBio" ~ "CO2 Emissions (MtCO2)",
                           grepl("energyFinalConsumBySec", param) ~ "Energy Consumption by Sector (EJ)",
                           grepl("energyFinalByFuelEJNoFeedstock", param) ~ "Energy Consumption by Fuel (EJ)",
                           param == "elecByTechTWh" ~ "Electricity Generation (TWh)",
                           param == "elecConsumByDemandSectorTWh" ~ "Electricity Consumption (TWh)",
                           param == "energyFinalSubsecByFuelBuildEJ" ~ "Building Energy Consumption by Fuel (EJ)",
                           param == "energyFinalSubsecBySectorBuildEJ" ~ "Building Energy Consumption by Sector (EJ)",
                           param == "energyFinalSubsecByFuelIndusEJ" ~ "Industry Energy Consumption by Fuel (EJ)",
                           T ~ param))

thailand_with_hist <- thailand_with_hist %>% 
  mutate(units = case_when(param == "pop" ~ "Population (millions)",
                           param == "gdp" ~ "GDP (Billion 1990 USD)",
                           param == "gdpPerCapita" ~ "GDP per Capita (Billion 1990 USD per person)",
                           param == "transportFreightVMTByFuel" ~ "Freight (million ton-km) by Fuel",
...
thailand$scenario <- factor(thailand$scenario,
                            levels = c("BAU", "POL", "CN2050", "CN2050+LUC"),
                            labels = c("1. Business as Usual", "2. Policies",
                                       "3. Carbon Neutral", "4. Carbon Neutral + LUC"))

scenRef_name <- "1. Business as Usual"
...
emissions_line_params <- c("emissCO2BySectorNoBio")
...
``` {r emiss-th, results = 'hide', eval = TRUE, echo = TRUE, warning = FALSE, error = FALSE, message = FALSE,fig.width = 20, fig.height = 5, out.width = "100%", fig.cap="National CO2 emissions by sector, GHG emissions by gas, and GHG emissions by sector in the reference scenario and the low and high policies scenarios"}

emissions_params <- c("emissCO2BySectorNoBio")

emissions_bar <- thailand %>% filter(param %in% emissions_params,
                                     region %in% c("All of Thailand", "MEA area")) %>%
  filter(class != "LUC") %>% 
  mutate(class = case_when(class %in% c("hydrogen", "refining") ~ "Industry",
                           T ~ class)) %>% 
  group_by(scenario, region, x, class, param, units) %>% 
  ...
  rchart::chart(save = F, 
                show = F,
                size_text = 19,
                scenRef = scenRef_name,
                summary_line = T,
                chart_type = "class_absolute",
                palette = c("BECCS" = "forestgreen"),
                ncol = 4)
...
``` {r emiss-diff-th, results = 'hide', eval = TRUE, echo = TRUE, warning = FALSE, error = FALSE, message = FALSE,fig.width = 32, fig.height = 8, out.width = "100%", fig.cap="Difference in national CO2 and GHG emissions by sector and gas between the reference scenario (left) and the low and high policy scenarios (right)"}
...
emissions_diff <- thailand %>% filter(param %in% emissions_params,
  ...
  mutate(class = case_when(class %in% c("hydrogen", "refining") ~ "Industry",
                           T ~ class)) %>% 
  group_by(scenario, region, x, class, units) %>% 
  summarize(value = sum(value)) %>% 
  ungroup() %>% 
  mutate(param = units) %>% 
  rchart::chart(save = F, 
...
```{r energy-th, results = 'hide', eval = TRUE, echo = TRUE, warning = FALSE, error = FALSE, message = FALSE, fig.width = 20, fig.height = 10, out.width = "100%", fig.cap="National final energy consumption by fuel and sector in the reference scenario and the low and high policies scenarios"}
energy_parameters <- c("energyFinalConsumBySecEJNoFeedstock", "energyFinalByFuelEJNoFeedstock")
...
```{r building-th, results = 'hide', eval = TRUE, echo = TRUE, warning = FALSE, error = FALSE, message = FALSE, fig.width = 26, fig.height = 12, out.width = "100%", fig.cap="National buildings sector final energy by fuel and  by service in the reference scenario and the low and high policies scenarios"}
buildings_params <- c("energyFinalSubsecByFuelBuildEJ", 
                      "energyFinalSubsecBySectorBuildEJ")

building_sector_colors <- c(
  "commercial ACMV" = "#b2e2e2",
  "commercial cooking" = "#66c2a4",
  "commercial lighting" = "#2ca25f",
  "commercial other" = "#006d2c",
  "residential ACMV" = "#fee8c8",
  "residential cooking" = "#fdd49e",
  "residential lighting" = "#fdbb84",
  "residential other" = "#fc8d59")
...
buildings <- thailand %>% 
  ...
  rchart::chart(save = F, 
                ...
                size_text = 19,
                scenRef = scenRef_name,
                palette = building_sector_colors,
                chart_type = c("class_absolute", "class_diff_absolute"),
                scales = "fixed",
                ncol = 4)
...
##### Transportation
```{r transport-th, results = 'hide', eval = TRUE, echo = TRUE, warning = FALSE, error = FALSE, message = FALSE, fig.width = 24, fig.height = 18, out.width = "100%", fig.cap="National transportation GHG emissions by mode and service output by mode and fuel in the reference scenario and the low and high policies scenarios"}
...
```{r transport-diff-th, results = 'hide', eval = TRUE, echo = TRUE, warning = FALSE, error = FALSE, message = FALSE, fig.width = 36, fig.height = 24, out.width = "100%", fig.cap="Difference in national transportation GHG emissions by mode and service output by mode and fuel between the reference scenario (left) and the low and high policy scenarios (right)"}

transport$`chart_class_diff_absolute_All of Thailand`
```

##### Industry
```{r industry-th, results = 'hide', eval = TRUE, echo = TRUE, warning = FALSE, error = FALSE, message = FALSE, fig.width = 24, fig.height = 7, out.width = "100%", fig.cap=" National industrial GHG emissions and final energy consumption by fuel in the reference scenario and the low and high policies scenarios"}
industry_params <- c("energyFinalSubsecByFuelIndusEJ")
...
``` {r emiss-mea, results = 'hide', eval = TRUE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE,fig.width = 20, fig.height = 5, out.width = "100%",  fig.cap="MEA area CO2 emissions by sector, GHG emissions by gas, and GHG emissions by sector in the reference scenario and the low and high policies scenarios"}

emissions_bar$`chart_class_MEA area`

```

``` {r emiss-diff-mea, results = 'hide', eval = TRUE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE,fig.width = 32, fig.height = 9, out.width = "100%", fig.cap="Difference in MEA area CO2 and GHG emissions by sector and gas between the reference scenario (left) and the low and high policy scenarios (right)"}
...
```{r energy-mea, results = 'hide', eval = TRUE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, fig.width = 24, fig.height = 12, out.width = "100%", fig.cap="MEA area final energy consumption by fuel and sector in the reference scenario and the low and high policies scenarios"}

energy$`chart_class_MEA area`
```

``` {r energy-diff-mea, results = 'hide', eval = TRUE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE,fig.width = 32, fig.height = 10, out.width = "100%", fig.cap="Difference in MEA area final energy consumption by fuel and sector between the reference scenario (left) and the low and high policy scenarios (right)"}
...
##### Buildings
```{r building-mea, results = 'hide', eval = TRUE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, fig.width = 21, fig.height = 8, out.width = "100%", fig.cap="MEA area buildings sector GHG emissions by service, final energy by fuel, and final energy by service in the reference scenario and the low and high policies scenarios"}

buildings$`chart_class_MEA area`
```

```{r building-diff-mea, results = 'hide', eval = TRUE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, fig.width = 38, fig.height = 12, out.width = "100%", fig.cap="Difference in MEA area buildings sector GHG emissions by service, final energy by fuel, and final energy by service between the reference scenario (left) and the low and high policy scenarios (right)"}

buildings$`chart_class_diff_absolute_MEA area`
```

##### Transportation
```{r transport-mea, results = 'hide', eval = TRUE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, fig.width = 24, fig.height = 18, out.width = "100%", fig.cap="MEA area transportation GHG emissions by mode and service output by mode and fuel in the reference scenario and the low and high policies scenarios"}
...
##### Industry
```{r industry-mea, results = 'hide', eval = TRUE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, fig.width = 24, fig.height = 8, out.width = "100%", fig.cap="MEA area industrial GHG emissions and final energy consumption by fuel in the reference scenario and the low and high policies scenarios"}

industry$`chart_class_MEA area`
```

```{r industry-diff-mea, results = 'hide', eval = TRUE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, fig.width = 34, fig.height = 6, out.width = "100%", fig.cap = "Difference in MEA area industrial GHG emissions and final energy consumption by fuel between the reference scenario (left) and the low and high policy scenarios (right)"}
...
# read in gcam data
gcam_co2_emiss_th <- thailand_with_hist %>%
  filter(scenario == scenRef_name,
         param == "emissCO2BySectorNoBio",
...
# no trn_intl version
gcam_co2_emiss_th_noInt <- thailand_with_hist %>% 
  filter(scenario == scenRef_name,
         param == "emissCO2BySectorNoBio",
         region == "All of Thailand",
         x >= 1990, class != "LUC",
...
gcam_elec_consump_th <- thailand_with_hist %>% 
  filter(scenario == scenRef_name,
         param == "elecConsumByDemandSectorTWh",
         region == "All of Thailand",
         x >= 2000) %>% 
  select(x, units, region, scenario, value, class) %>% 
...
# read  gcam data
gcam_final_energy_fuel <- thailand_with_hist %>% 
  filter(scenario == scenRef_name,
         region == "All of Thailand",
...
gcam_final_energy_fuel_noFeed <- thailand_with_hist %>% 
  filter(scenario == scenRef_name,
         region == "All of Thailand",
...
######### by sector ##########
# read in gcam data
gcam_final_energy_sector <- thailand_with_hist %>% 
  filter(scenario == scenRef_name,
         region == "All of Thailand",
...
# version without industrial feedstocks
gcam_final_energy_sector_noFeed <- thailand_with_hist %>% 
  filter(scenario == scenRef_name,
         region == "All of Thailand",
...
final_energy_sector_chart_local$chart_class +
  ggplot2::ggtitle("") + ggplot2::ylab("Energy (EJ)") + ggplot2::ylim(c(0,5))
```
...
final_energy_fuel_chart_local$chart_class +
  ggplot2::ggtitle("") + ggplot2::ylab("Energy (EJ)") + ggplot2::ylim(c(0,5.5))
```
...
# 
# # read in gcam data
# gcam_ghg_mode <- thailand_with_hist %>% 
#   filter(scenario == scenRef_name,
#          region == "All of Thailand",
...
gcam_elec_consump_mea <- thailand_with_hist %>% 
  filter(scenario == scenRef_name,
         param == "elecConsumByDemandSectorTWh",
         region == "MEA area",
         x >= 2000) %>% 
