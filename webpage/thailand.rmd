...
```{r mandatory national policy table, results = 'show'}
library(pander)
myTable = data.frame(
...
pander::pander(myTable, keep.line.breaks = TRUE, style = "grid", justify = 'left', split.tables=Inf, caption = "Table 5.1.1. Policies included in the Mandatory National Policies scenario.")
```
...
```{r recent national trends pcgdp baht, fig.dim=c(8,4), fig.cap="Figure 9.1.3.1. **Growth rate of national per capita GDP from 2011 to 2019**"}
...
local_national_transport_ghg <- read.csv(
  "data/Thailand/outputs/initial_diagnostics/local_transport_ghg_emissions_mode.csv",
...
gcam_buildings_final_energy_fuel <- process_gcam_data(
  read.csv("data/Thailand/outputs/initial_diagnostics/queryoutall_building_final_energy_service_fuel.csv", skip = 1, check.names = F),
  processed = F, input, sector
)
