...
pop_inputs <- read.csv("data/thailand_bangkok_nonthaburi_samutprakan_pop.csv")
...
Thailand's National Statistical Office (NSO) has reported annual GDP both nationally and by province for the period from 2010 to 2019. The plot below shows national GDP in this period.

```{r recent national trends gdp, results = "hide", echo=FALSE, out.width="70%", fig.align='center'}
gdp_inputs <- read.csv("data/thailand_bangkok_nonthaburi_samutprakan_gdp.csv")
gdp_inputs$region <- factor(gdp_inputs$region, 
                            levels = c("Thailand", "Bangkok", "Samut Prakan",
                                       "Nonthaburi", "Rest of Thailand"))

gdp_recent_chart_national <- rchart::chart(
  dplyr::mutate(
    dplyr::filter(gdp_inputs, 
                  year >= 2010, year <= 2019,
                  region == "Thailand"),
      value = value/1000000000),
  save = F, break_interval = 2, aspect_ratio = 0.5)

gdp_recent_chart_national$chart_param +
...
```{r recent MEA province trends gdp, results = "hide", echo=FALSE, out.width="70%", out.extra='style="float:right; padding:10px"'}
