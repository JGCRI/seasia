...
<span>- KL 2020 population value was taken from the Key Findings Population and Housing Census of Malaysia 2020: Administrative District. All other values were extrapolated and calculated using the growth rates for Bangkok </span><br/>
...
<span>- Kuala Lumpur 2020: [Census of Malaysia 2020](https://www.dosm.gov.my/v1/index.php?r=column/cthemeByCat&cat=500&bul_id=WEFGYlprNFpVcUdWcXFFWkY3WHhEQT09&menu_id=L0pheU43NWJwRWVSZklWdzQ4TlhUUT09)</span><br/>
</td>
</tr>
...
<td rowspan="3">GDP</td>
<td>
<span>- KL's 2020 GDP value is sourced from the State Socioeconomic Report 2020. Bangkok annual growth rates are applied to extrapolate historical and future values</span><br/>
...
To estimate population values, we begin with annual data from the UN for Malaysia's population; we use historical data and a medium population projection scenario. For Kuala Lumpur, we use 2020 census data as our base year. We then calculate annual growth rates using growth rates for Bangkok over the same period and apply these growth rates to extrapolate Kuala Lumpur population values. To find "Rest of Malaysia" values, we subtract Kuala Lumpur population from Malaysia values.
...
GDP per capita, in units of thousand 2005 USD, is shown below. 
...
final_energy <- fread("../data/Malaysia/gcam_queries/gcam_query_energy.csv") %>% as_tibble()
...
elec <- fread("../data/Malaysia/gcam_queries/gcam_query_electricity.csv")
...
co2_sector <- fread("../data/Malaysia/gcam_queries/gcam_query_co2_sector.csv")
...
co2_buildings <- fread("../data/Malaysia/gcam_queries/gcam_query_co2_buildings.csv")
...
building_energy <- fread("../data/Malaysia/gcam_queries/gcam_query_building_energy.csv")
...
building_energy_sector <- fread("../data/Malaysia/gcam_queries/gcam_query_building_energy_sector.csv")
...
co2_subsector <- fread("../data/Malaysia/gcam_queries/gcam_query_co2_subsector.csv")
