...
xml = c("configuration_malaysia_bau.xml",
        "configuration_malaysia_policies.xml",
        "configuration_malaysia_carbon_neutral.xml")
...
data.frame(
  ...
  mutate("Files Used" = cell_spec(c("trn_modal_shift.xml"),
                                   "html", link = dt_url)) %>% 
  mutate(Description = c("Increases ratio of public to private transportation to 80-20 in Kuala Lumpur and 55-45 in the rest of the country in 2050")) %>%
  kable("html", escape = FALSE, col.names = c("Scenario", "Files Used", "Description")) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive")) %>%
  row_spec(0, background = "#2A2A2A", color = "white")
...
4. To make custom adjustments, change the `share-weight` for `year="2100"` in a given `tranSubsector`. Increasing the `share-weight` will result in a greater share of the mode that was adjusted, while decreasing it will result in less.
5. Save the xml and then point to it in your configuration file by adding the line: 
...
data.frame(
  ...
  dplyr::mutate("Files Used" = cell_spec(c("elec_gen_shares.xml", "elec_hydro.xml"),
                                          "html", link=dt_url),
                 Description = c("Sets shares for renewable electricity generation technologies",
...
<li>Create a `policy-portfolio-standard` for the technology (use current XML as an example). </li>
<li>Add this technology as a `minicam-energy-input` within each `supplysector` or `pass-through-sector`/`subsector`/`stub-technology` for every `period`, and adjust the `current-coef`.</li>
<li>Create a `res-secondary-output` for this technology, where applicable. For example, a `res-secondary-output` is created for solar in `supplysector`:electricity/`subsector`:solar/`stub-technology`:PV, `supplysector`:electricity/`subsector`:solar/`stub-technology`:PV_storage, `supplysector`:elect_td_bld/`subsector`:rooftop_pv/`stub-technology`:rooftop_pv, `pass-through-sector`:elec_CSP/`subsector`:CSP/`stub-technology`:CSP (recirculating), `pass-through-sector`:elec_CSP/`subsector`:CSP/`stub-technology`:CSP (dry_hybrid),
`pass-through-sector`:elec_CSP_storage/`subsector`:CSP_storage/`stub-technology`:CSP_storage (recirculating), and
`pass-through-sector`:elec_CSP_storage/`subsector`:CSP_storage/`stub-technology`:CSP_storage (dry_hybrid), because these are all solar-related technologies.
</ul>
<p style='margin-left:1em;'>
...
# add scenario names, file names, and descriptions of files to this table.
...
Note that due to size limitations on GitHub, the data used here has already been processed, but we include the steps to run it on your own in the following chunk if desired.
...
all_data_buildings_aggregate <- malaysia %>%
  ...
  dplyr::mutate(class = dplyr::case_when(
    ...
    # Rename commercial sectors for consistency
...
```{r rchart-socio, results = 'hide', eval = TRUE, echo = TRUE, warning = FALSE, error = FALSE, message = FALSE,fig.topcaption=TRUE, fig.cap="BAU GDP, GDP per capita, and population in Malaysia and Kuala Lumpur"}
...
``` {r rchart-emiss-plot, results = 'hide', eval = TRUE, echo = TRUE, warning = FALSE, error = FALSE, message = FALSE, fig.topcaption=TRUE, fig.cap="Direct BAU CO~2~ emissions by sector in Malaysia and Kuala Lumpur"}
...
```{r co2-figs-1, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="Total CO~2~ emissions by scenario in Malaysia"}
knitr::include_graphics("./markdown_figs/chart_param_co2_lines.png")

```

```{r co2-figs-2, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="CO~2~ emissions by sector in Malaysia"}
knitr::include_graphics("./markdown_figs/chart_class_co2_emiss.png")

```

```{r co2-figs-3, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="BAU CO~2~ emissions (left) and the difference between BAU and the Policies and Carbon Neutral scenarios"}
...
```{r co2-figs-kl-1, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="Total CO~2~ emissions by scenario in Kuala Lumpur"}
knitr::include_graphics("./markdown_figs/chart_param_co2_lines_kl.png")

```

```{r co2-figs-kl-2, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="CO~2~ emissions by sector in Kuala Lumpur" }
knitr::include_graphics("./markdown_figs/chart_class_co2_emiss_kl.png")

```

```{r co2-figs-kl-3, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="BAU CO~2~ emissions (left) and the difference between BAU and the Policies and Carbon Neutral scenarios"}
...
```{r energy-figs-1, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="Energy consumption by sector in Malaysia"}
knitr::include_graphics("./markdown_figs/chart_class_energy_consum.png")

```

```{r energy-figs-2, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="BAU energy consumption (left) and the difference between BAU and the Policies and Carbon Neutral scenarios"}
knitr::include_graphics("./markdown_figs/chart_class_diff_absolute_energy_consum.png")

```

```{r energy-figs-3, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="Energy consumption by fuel in Malaysia"}
knitr::include_graphics("./markdown_figs/chart_class_energy_fuel.png")

```

```{r energy-figs-4, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="BAU energy consumption (left) and the difference between BAU and the Policies and Carbon Neutral scenarios"}
...
```{r energy-figs-kl-1, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="Energy consumption by sector in Kuala Lumpur"}
knitr::include_graphics("./markdown_figs/chart_class_energy_consum_kl.png")

```

```{r energy-figs-kl-2, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="BAU energy consumption (left) and the difference between BAU and the Policies and Carbon Neutral scenarios"}
knitr::include_graphics("./markdown_figs/chart_class_diff_absolute_energy_consum_kl.png")

```

```{r energy-figs-kl-3, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="Energy consumption by fuel in Kuala Lumpur"}
knitr::include_graphics("./markdown_figs/chart_class_energy_fuel_kl.png")

```

```{r energy-figs-kl-4, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="BAU energy consumption (left) and the difference between BAU and the Policies and Carbon Neutral scenarios"}
...
```{r elec-figs-1, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="Electricity generation by fuel in Malaysia"}
knitr::include_graphics("./markdown_figs/chart_class_elec_gen.png")

```

```{r elec-figs-2, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="BAU electricity generation (left) and the difference between BAU and the Policies and Carbon Neutral scenarios"}
...
```{r bld-figs-1, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="Residential building energy consumption by sector in Malaysia"}
knitr::include_graphics("./markdown_figs/chart_class_resid_bld.png")

```

```{r bld-figs-2, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="BAU energy consumption (left) and the difference between BAU and the Policies and Carbon Neutral scenarios"}
knitr::include_graphics("./markdown_figs/chart_class_diff_absolute_resid_bld.png")

```

```{r bld-figs-3, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="Commercial building energy consumption by sector in Malaysia"}
knitr::include_graphics("./markdown_figs/chart_class_comm_bld.png")

```

```{r bld-figs-4, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="BAU energy consumption (left) and the difference between BAU and the Policies and Carbon Neutral scenarios"}
...
```{r bld-figs-kl-1, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="Residential building energy consumption by sector in Kuala Lumpur"}
knitr::include_graphics("./markdown_figs/chart_class_resid_bld_kl.png")

```

```{r bld-figs-kl-2, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="BAU energy consumption (left) and the difference between BAU and the Policies and Carbon Neutral scenarios"}
knitr::include_graphics("./markdown_figs/chart_class_diff_absolute_resid_bld_kl.png")

```

```{r bld-figs-kl-3, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="Commercial building energy consumption by sector in Kuala Lumpur"}
knitr::include_graphics("./markdown_figs/chart_class_comm_bld_kl.png")

```

```{r bld-figs-kl-4, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="BAU energy consumption (left) and the difference between BAU and the Policies and Carbon Neutral scenarios"}
...
```{r mode-figs-1, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="Freight vehicle kilometers traveled by mode in Malaysia"}
knitr::include_graphics("./markdown_figs/chart_class_freight_mode.png")

```

```{r mode-figs-2, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="BAU freight vehicle kilometers traveled (left) and the difference between BAU and the Policies and Carbon Neutral scenarios"}
knitr::include_graphics("./markdown_figs/chart_class_diff_absolute_freight_mode.png")

```

```{r mode-figs-3, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="Passenger vehicle kilometers traveled by mode in Malaysia"}
knitr::include_graphics("./markdown_figs/chart_class_pass_mode.png")

```

```{r mode-figs-4, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="BAU passenger vehicle kilometers traveled (left) and the difference between BAU and the Policies and Carbon Neutral scenarios"}
...
```{r mode-figs-kl-1, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="Freight vehicle kilometers traveled in Kuala Lumpur"}
knitr::include_graphics("./markdown_figs/chart_class_freight_mode_kl.png")

```

```{r mode-figs-kl-2, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="BAU freight vehicle kilometers traveled (left) and the difference between BAU and the Policies and Carbon Neutral scenarios"}
knitr::include_graphics("./markdown_figs/chart_class_diff_absolute_freight_mode_kl.png")

```

```{r mode-figs-kl-3, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="Passenger vehicle kilometers traveled in Kuala Lumpur"}
knitr::include_graphics("./markdown_figs/chart_class_pass_mode_kl.png")

```

```{r mode-figs-kl-4, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="BAU passenger vehicle kilometers traveled (left) and the difference between BAU and the Policies and Carbon Neutral scenarios"}
...
```{r trn-figs-1, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="Freight energy consumption by fuel in Malaysia"}
knitr::include_graphics("./markdown_figs/chart_class_freight_fuel.png")

```

```{r trn-figs-2, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="BAU energy consumption (left) and the difference between BAU and the Policies and Carbon Neutral scenarios"}
knitr::include_graphics("./markdown_figs/chart_class_diff_absolute_freight_fuel.png")

```

```{r trn-figs-3, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="Passenger energy consumption by fuel in Malaysia"}
knitr::include_graphics("./markdown_figs/chart_class_pass_fuel.png")

```

```{r trn-figs-4, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="BAU energy consumption (left) and the difference between BAU and the Policies and Carbon Neutral scenarios"}
...
```{r trn-figs-kl-1, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="Freight energy consumption by fuel in Kuala Lumpur"}
knitr::include_graphics("./markdown_figs/chart_class_freight_fuel_kl.png")

```

```{r trn-figs-kl-2, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="BAU energy consumption (left) and the difference between BAU and the Policies and Carbon Neutral scenarios"}
knitr::include_graphics("./markdown_figs/chart_class_diff_absolute_freight_fuel_kl.png")

```

```{r trn-figs-kl-3, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="Passenger energy consumption by fuel in Kuala Lumpur"}
knitr::include_graphics("./markdown_figs/chart_class_pass_fuel_kl.png")

```

```{r trn-figs-kl-4, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="BAU energy consumption (left) and the difference between BAU and the Policies and Carbon Neutral scenarios"}
...
<!-------------------------->
#### Industry Energy by Fuel {.tabset}
<!-------------------------->
***
...
```{r indus-figs-1, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="Industry energy consumption by fuel in Malaysia"}
knitr::include_graphics("./markdown_figs/chart_class_indus_fuel.png")

```

```{r indus-figs-2, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="BAU energy consumption (left) and the difference between BAU and the Policies and Carbon Neutral scenarios"}
...
```{r indus-figs-kl-1, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="Industry energy consumption by fuel in Kuala Lumpur"}
knitr::include_graphics("./markdown_figs/chart_class_indus_fuel_kl.png")

```

```{r indus-figs-kl-2, eval = 'TRUE', echo = 'TRUE', results = "show", fig.topcaption=TRUE, fig.cap="BAU energy consumption (left) and the difference between BAU and the Policies and Carbon Neutral scenarios"}
...
<span>- KL census data gives a value for 2010. 2030 and 2050 values came from the Kuala Lumpur Climate Action Plan 2050. Values for 2011-2049 were interpoalted, all other values were extrapolated and calculated using the growth rates for Malaysia in total</span><br/>
...
<td rowspan="3">GDP</td>
<td>
<span>- Malaysia annual growth rates are applied to KL's 2020 GDP value to extrapolate historical values. Future values calculated using growth rates from the Kuala Lumpur Climate Action Plan 2050, with 2050 growth rates being applied out to 2100</span><br/>
...
```{r local-emissions-kl, results = 'hide', eval = FALSE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE}
# TODO figure out why this doesn't line up
