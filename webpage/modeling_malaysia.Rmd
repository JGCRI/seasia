...
<p style='margin-left:1em;'>
The goal of this policy is to phase out internal combustion engine (ICE) passenger vehicles from the market. The difference between the low and high policies is the year in which ICE vehicles are completely phased out of the market.
...
`<Value name = "scen">../input/addons/malaysia/trn_ICE_low.xml</Value>`
</p>
</p>
...
data.frame(
  ...
  mutate(Description = c("Decreases shell conductance from 3.75 in 2020 to 0.487 (residential)/ 0.375 (commercial) in 2070 at an annual rate of 4% (residential)/ 4.5% (commercial)",
                         ...
                         "Follows the 'high' scenario trajectory in KL and the 'low' scenario trajectory in the rest of Malaysia")) %>%
  kable("html", escape = FALSE, col.names = c("Scenario", "Files Used", "Description")) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive")) %>%
  row_spec(0, background = "#2A2A2A", color = "white")
...
<p style='margin-left:1em;'>
1. Create a folder in the input directory eg. `./gcam-core/input/addons/malaysia`.
2. Download the example XML file ( [elec_gen_low.xml](https://github.com/JGCRI/seasia/blob/main/modeling/malaysia/sectoral_policies/elec_gen_low.xml) and the subsidy link file [elec_gen_link.xml](https://github.com/JGCRI/seasia/blob/main/modeling/malaysia/sectoral_policies/elec_gen_link.xml) to the folder. (Click on `Raw` and then `right-click` and select `save-as`.)
...
[elec_hydro_low.xml](https://github.com/JGCRI/seasia/blob/main/modeling/malaysia/sectoral_policies/elec_hydro_low) to the folder. 
...
`<Value name = "scen">../input/addons/malaysia/elec_gen_low.xml</Value>`
`<Value name = "scen">../input/addons/malaysia/elec_hydro_low.xml</Value>`
